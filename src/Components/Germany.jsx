...
import { getGermany } from "../Services/api";
...
function Germany(){
    ...
    const app = async() =>{
        try{
            ...
            setLoading(true)
            const getGer = await getGermany(page);
            setGermany(getGer.filter((item) => item.poster_path));
        }catch(err){
    ...
    return(
        ...
        <div className="contain home-contain">
         ...
         <div className="germanytitle">
            <h1>Germany Movies & TV Shows Online Free</h1>
            <p style={{color:"gray"}}>Watch movies and TV shows from Germany free in HD on Lijinflix. 1,201 titles available — no signup or subscription. Discover the best of Germany and television, with English subtitles available for most titles.</p>
         </div>
         <div className="germanycontain">
            {germany.map((data) =>(
                <div className="germanycard" key={`${data.type}-${data.id}`}>
                <img
                    onClick={() =>handleClick(data)}
                    onError={() => setGermany((items) => items.filter((item) => `${item.type}-${item.id}` !== `${data.type}-${data.id}`))}
                    className="germanyposter"
                    src={`https://image.tmdb.org/t/p/w500${data.poster_path}`}
                    alt={data.name || data.title || "No poster available"}
                />
                <div className="germanyoverlay">
