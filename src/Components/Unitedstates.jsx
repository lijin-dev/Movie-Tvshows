...
import { getUs } from "../Services/api";
...
function Unitedstate(){
    ...
    const app = async() =>{
        try{
            setLoading(true);
            const getAmerica = await getUs(page);
            setAmerica(getAmerica);
        }catch(err){
    ...
    return(
        ...
        <div className="contain home-contain">
         ...
         <div className="ustitle">
            <h1>United States Movies & TV Shows Online Free</h1>
            <p style={{color:"gray"}}>Watch movies and TV shows from United States free in HD on Nunflix. 1,201 titles available — no signup or subscription. Discover the best of United States cinema and television, with English subtitles available for most titles.</p>
         </div>
         <div className="uscontain">
            {america.map((data) =>(
                <div key={`${data.type}-${data.id}`} className="us-card" onClick={() => handleClick(data)}>
                <img className="usposter" src={`https://image.tmdb.org/t/p/w500${data.poster_path}`} alt={data.name || data.title} />
