...
import '../CSS/Crime.css'

function Crime(){
 
    const[crime, setCrime] = useState([]);
    const[page, setPage] = useState(() => {
        const savedPage = Number(localStorage.getItem("crimePage"));
    ...
    useEffect(() => {
        localStorage.setItem("crimePage", String(page));
    }, [page]);
    ...
    const app = async() =>{
        try{
            setLoading(true);
            const getcriminal = await getCrime(page);
            setCrime(getcriminal);
        }catch(err){
    ...
    return(
        ...
        <div className="contain home-contain">
         ...
         <div className="crimetitle">
            <h1>Crime Movies & TV Shows Online Free</h1>
            <p style={{color:"gray"}}>Stream the best Crime movies and TV shows free on Nunflix in HD. Browse 1,201 Crime titles — no signup, no subscription required. Updated daily with the newest Crime releases.</p>
         </div>
         <div className="crimecontain">
            {crime.map((data) =>(
            <div className="crimecard" key={`${data.type}-${data.id}`}>
                <img onClick={() =>handleClick(data)} className="crimeposter" src={`https://image.tmdb.org/t/p/w500${data.poster_path}`} alt={data.name || data.title} />
