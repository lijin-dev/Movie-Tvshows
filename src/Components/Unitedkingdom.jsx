...
import { getUk } from "../Services/api";
import '../CSS/Unitedkingdom.css';
    ...
    return(
        ...
        <div className="contain home-contain">
         ...
         <div className="uktitle">
            <h1>United Kingdom Movies & TV Shows Online Free</h1>
            <p style={{color:"gray"}}>Watch movies and TV shows from United Kingdom free in HD on Lijinflix. 1,201 titles available — no signup or subscription. Discover the best of United Kingdom cinema and television, with English subtitles available for most titles.</p>
         </div>
         <div className="ukcontain">
            {uk.map((data) =>{
                const titleText = data.title || data.name;
                const nameText = data.name && data.name !== titleText ? data.name : null;

                return (
                    <div className="ukcard" key={`${data.type}-${data.id}`} onClick={() => handleClick(data)}>
                        <div className="ukposterwrap">
                            <img className="ukposter" src={`https://image.tmdb.org/t/p/w500${data.poster_path}`} alt={titleText || "Poster"} />
                            <div className="ukoverlay">
                                {titleText && <p className="ukmeta ukmeta-title">{titleText}</p>}
                                {nameText && <p className="ukmeta">{nameText}</p>}
                                {data.release_date && <p className="ukmeta">{data.release_date}</p>}
                                {data.first_air_date && <p className="ukmeta">{data.first_air_date}</p>}
                            </div>
                        </div>
                    </div>
                );
