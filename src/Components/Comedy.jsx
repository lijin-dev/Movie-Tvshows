...
function Comedy(){
    ...
    const[page, setPage] = useState(() => {
        const savedPage = Number(localStorage.getItem("comedyPage"));
    ...
    useEffect(() => {
        localStorage.setItem("comedyPage", String(page));
    }, [page]);

    const app = async() =>{
        try{
            setLoading(true)
            const getJoke = await getComedy(page);
            setComedy(getJoke);
        }catch(err){
    ...
    return(
        ...
        <div className="comedytitle">
            <h1>Comedy Movies & TV Shows Online Free</h1>
            <p style={{color:"gray"}}>Stream the best Comedy movies and TV shows free on Nunflix in HD. Browse 1,201 Comedy titles — no signup, no subscription required. Updated daily with the newest Comedy releases.</p>
        </div>
        <div className="comedycontain">
            {comedy.map((data) =>(
            <div className="comedycard" key={`${data.type}-${data.id}`}>
                <img onClick={() => handleClick(data)} className="comedyposter" src={`https://image.tmdb.org/t/p/w500${data.poster_path}`} alt={data.title || data.name || ""} />
                <div className="comedyinfo">
