...
import { getCanada } from "../Services/api";
...
function Canada(){
    ...
    const app = async() =>{
        try{
            setLoading(true)
            const getCan  = await getCanada(page);
            setCanada(getCan);
        }catch(err){
    ...
    return(
        ...
        <div className="contain home-contain">
         ...
         <div className="canadatitle">
            <h1>Canada Movies & TV Shows Online Free</h1>
            <p style={{color:"gray"}}>Watch movies and TV shows from Canada free in HD on Lijinflix. 1,201 titles available — no signup or subscription. Discover the best of Canada and television, with English subtitles available for most titles.</p>
         </div>
         <div className="canadacontain">
            {canada.map((data) =>(
                <div className="canadacard" key={`${data.type}-${data.id}`}>
                <img onClick={() =>handleClick(data)} className="canadaposter" src={`https://image.tmdb.org/t/p/w500${data.poster_path}`} alt={data.name || data.title} />
                <div className="canadadetails">
                    <p>{data.name || data.title}</p>
                    {data.name && data.title && <p>{data.title}</p>}
